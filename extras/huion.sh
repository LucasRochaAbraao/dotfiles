...
if [ $(echo "$CONNECTED_MONITORS" | wc -l) -gt 1 ]; then
    ...
    echo "Randomly mapping the graphics tablet to '$MONITOR'."
else
    MONITOR=$CONNECTED_MONITORS
    echo "Monitor detected on: $MONITOR."
fi
...
if test -n "$STYLUS1_ID"; then
    echo "Mapping huion tablet to monitor '$MONITOR'."
...
if ! [ -n "$STYLUS1_ID" ] && ! [ -n "$STYLUS2_ID" ]; then
    echo "Couldn't find any graphics tablet or stylus. Make sure it's plugged in correctly and check 'xinput'."
else
    echo "Possible Stylus IDs. In case you want to change the active monitor (current:$MONITOR), run:"
    echo "xinput map-to-output [$STYLUS1_ID or $STYLUS2_ID] [a monitor from the list above]"
fi
